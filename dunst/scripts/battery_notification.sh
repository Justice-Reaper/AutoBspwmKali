...
if [ "$battery_level" -le "$warning_level" ] && [ "$(cat $battery_warning_file)" != "True" ] && [ "$battery_charging" -eq 0 ]; then
    ...
    notify-send "Low Battery" "${battery_level}% of battery remaining" -c critical-battery -i "/home/user_replace/.config/dunst/icons/battery-alert.png" -r 9991
    ...
    echo 'False' > "$battery_fully_charged_file"
    echo 'False' > "$battery_discharging_file"
    notify-send "Charging" "${battery_level}% of battery charged" -c normal-battery -i "/home/user_replace/.config/dunst/icons/battery-charging.png" -r 9991
    ...
    echo 'False' > "$battery_warning_file"
    echo 'False' > "$battery_discharging_file"
    notify-send "Battery Charged" "Battery is fully charged" -c normal-battery -i "/home/user_replace/.config/dunst/icons/battery-fully-charged.png" -r 9991
fi
