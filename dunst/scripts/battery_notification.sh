...
battery_discharging_file=/home/user_replace/.config/bin/battery-discharging
battery_charging_file=/home/user_replace/.config/bin/battery-charging
battery_warning_file=/home/user_replace/.config/bin/battery-warning
battery_fully_charged_file=/home/user_replace/.config/bin/battery-fully-charged
...
if [ "$battery_level" -le "$warning_level" ] && [ "$(cat $battery_warning_file)" != "True" ] && [ "$battery_charging" -eq 0 ]; then
    ...
    notify-send "Low Battery" "${battery_level}% of battery remaining" -u critical -c battery-critical -i "/home/user_replace/.config/dunst/icons/battery-warning.png" -r 9991
    ...
    echo 'False' > "$battery_fully_charged_file"
    echo 'False' > "$battery_discharging_file"
    notify-send "Charging" "${battery_level}% of battery charged" -u normal -c battery-normal -i "/home/user_replace/.config/dunst/icons/battery-charging.png" -r 9991
    ...
    echo 'False' > "$battery_warning_file"
    echo 'False' > "$battery_discharging_file"
    notify-send "Battery Charged" "Battery is fully charged" -u normal -c battery-normal -i "/home/user_replace/.config/dunst/icons/battery-fully-charged.png" -r 9991
fi
