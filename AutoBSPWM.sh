...
if [ -x "/usr/bin/kali-optimus" ]; then
    while true; do
        read -p "$(echo -e "\e[33m[*]\e[0m Do you want to run 'kali-optimus' on the system? (YES/NO): ")" response
        response=$(echo "$response" | tr '[:upper:]' '[:lower:]')
        if [ "$response" = "yes" ] || [ "$response" = "y" ]; then
            echo -e "\e[32m[*]\e[0m Running 'kali-optimus' ...\n"
            kali-optimus
            break
        elif [ "$response" = "no" ] || [ "$response" = "n" ]; then
...
apt install meson libglib2.0-dev libjson-glib-dev libpolkit-agent-1-dev libgtk-3-dev dunst papirus-icon-theme imagemagick feh xclip bspwm sxhkd suckless-tools fastfetch polybar betterlockscreen bat lsd fzf flameshot rofi kitty zsh pulseaudio-utils x11-utils moreutils -y
...
rm -rf /home/$input_username/.config/rofi
...
# CONFIGURING POLKIT AGENT
echo -e "\e[32m[*]\e[0m Configuring polkit agent ...\n"
chmod 755 polkit-agent/*
cp polkit-agent/* /usr/local/bin
latest_version=$(curl -s "https://api.github.com/repos/OmarCastro/cmd-polkit/releases/latest" | grep "tag_name" | cut -d '"' -f 4)
wget https://github.com/OmarCastro/cmd-polkit/archive/refs/tags/${latest_version}.tar.gz -O cmd-polkit.tar.gz
tar -xf cmd-polkit.tar.gz
cd cmd-polkit-*/
meson setup build
...
su $input_username -c "betterlockscreen -u /home/$input_username/Wallpapers/wallpaper-1.jpg"
...
chmod +x /home/$input_username/.config/rofi/power-menu/power-menu.sh
chmod +x /home/$input_username/.config/rofi/scope-manager/scope-manager.sh
sed -i "s/user_replace/$input_username/g" /home/$input_username/.config/rofi/power-menu/*
sed -i "s/user_replace/$input_username/g" /home/$input_username/.config/rofi/launcher/*
sed -i "s/user_replace/$input_username/g" /home/$input_username/.config/rofi/scope-manager/*
...
touch /home/$input_username/.config/bin/scope  
...
picom_installation(){
    ...
    while true; do
        ...
        if [ "$response" = "yes" ] || [ "$response" = "y" ]; then
            ...
            sed -i '0,/border-radius:               10px;/s/border-radius:               10px;/border-radius:               0px;/' /home/$input_username/.config/rofi/power-menu/style.rasi
...
desktop_shortcuts_configuration(){
    while true; do
        ...
        response=$(echo "$response" | tr '[:upper:]' '[:lower:]')
        sed -i '/Windows\/Fn + F2.*→.*Turn down the brightness/d' /usr/bin/show-help-panel
        sed -i '/Windows\/Fn + F3.*→.*Turn up the brightness/,+1d' /usr/bin/show-help-panel
        if [ "$response" = "windows" ]; then
            echo -e "\e[32m[*]\e[0m Configuring Windows key ...\n"
            sed -i 's|/Fn||g' /usr/bin/show-help-panel
            break
        elif [ "$response" = "fn" ]; then
            echo -e "\e[32m[*]\e[0m Configuring FN key ...\n"
            sed -i 's|Windows/||g' /usr/bin/show-help-panel
...
redshift_installation(){
    ...
    touch /home/$input_username/.config/bin/color-temperature-kelvin
    touch /home/$input_username/.config/bin/color-temperature-percentage
    touch /home/$input_username/.config/bin/redshift-status
    echo 'Off' > /home/$input_username/.config/bin/redshift-status
    echo '100' > /home/$input_username/.config/bin/color-temperature-percentage
    echo '6500' > /home/$input_username/.config/bin/color-temperature-kelvin
}
...
burpsuite_professional_installation(){
    ...
    latest_version=$(curl -s "https://portswigger.net/burp/releases" | grep -oP '\d{4}\.\d+(\.\d+)?' | sort -rV | head -n 1)
    curl -s "https://portswigger.net/burp/releases/download?product=desktop&version=$latest_version&type=Jar" -o "burpsuite_pro_v$latest_version.jar"
    (java -jar loader.jar) &
...
virtual_machine_configuration(){
    echo -e "\e[32m[*]\e[0m Configuring polybar ...\n"
    rm -f /home/$input_username/.config/polybar/laptop-config.ini
    rm -f /home/$input_username/.config/polybar/desktop-config.ini
    cd /home/$input_username/.config/polybar
    mv virtual-machine-config.ini config.ini
    cd "$installation_folder"

    rm -f /home/$input_username/.config/polybar/scripts/increase-brightness.sh 
    rm -f /home/$input_username/.config/polybar/scripts/decrease-brightness.sh 
    rm -f /home/$input_username/.config/polybar/scripts/brightness-control.sh
    rm -f /home/$input_username/.config/polybar/scripts/color-temperature-control.sh
...
desktop_configuration(){
    echo -e "\e[32m[*]\e[0m Configuring polybar ...\n"
    rm -f /home/$input_username/.config/polybar/laptop-config.ini
    rm -f /home/$input_username/.config/polybar/virtual-machine-config.ini
    cd /home/$input_username/.config/polybar
    mv desktop-config.ini config.ini
    cd "$installation_folder"
...
laptop_configuration(){
    ...
    rm -f /home/$input_username/.config/polybar/virtual-machine-config.ini
    cd /home/$input_username/.config/polybar
    mv laptop-config.ini config.ini
    cd "$installation_folder"
    ...
    touch /home/$input_username/.config/bin/battery-discharging  
    touch /home/$input_username/.config/bin/battery-charging
    touch /home/$input_username/.config/bin/battery-warning
    touch /home/$input_username/.config/bin/battery-fully-charged  
}
...
while true; do
    ...
    if [ "$response" = "yes" ] || [ "$response" = "y" ]; then
        ...
        sed -i '/Windows + Shift + O\s*→\s*Open Obsidian/d' /usr/bin/show-help-panel
        break
    else
        echo -e "\e[31m[*]\e[0m Invalid response. Please reply 'YES' or 'NO'.\n"
    fi
...
while true; do
    ...
    if [ "$response" = "yes" ] || [ "$response" = "y" ]; then
        ...
        sed -i '/Windows + Shift + A\s*→\s*Open Postman/d' /usr/bin/show-help-panel
        break
    else
        echo -e "\e[31m[*]\e[0m Invalid response. Please reply 'YES' or 'NO'.\n"
    fi
...
while true; do
    ...
    if [ "$response" = "yes" ] || [ "$response" = "y" ]; then
        ...
        sed -i '/Windows + Shift + V\s*→\s*Open VSCode/d' /usr/bin/show-help-panel
        break
    else
        echo -e "\e[31m[*]\e[0m Invalid response. Please reply 'YES' or 'NO'.\n"
    fi
...
while true; do
    ...
    if [ "$response" = "yes" ] || [ "$response" = "y" ]; then
        ...
        sed -i '/Windows + Shift + E\s*→\s*Open Pycharm/d' /usr/bin/show-help-panel
